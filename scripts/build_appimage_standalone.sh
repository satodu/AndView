#!/bin/bash
# Script para criar AppImage standalone do AndView (com ADB e scrcpy incluídos)
...
echo "====================================="
echo "  Criador de AppImage Standalone - AndView"
echo "====================================="
echo ""

APP_NAME="AndView"
APP_VERSION="0.0.1"
BUILD_DIR="$PROJECT_ROOT/build/AppImage-Standalone"
...
Version=1.0
Type=Application
Name=AndView (Standalone)
Comment=Android Device Manager with WiFi Connection Support (Standalone)
...
cd "$BUILD_DIR"
ARCH=x86_64 appimagetool "$APPDIR" "${APP_NAME}-${APP_VERSION}-standalone-x86_64.AppImage"
...
if [ $? -eq 0 ]; then
    ...
    echo "===================================="
    echo ""
    echo "📦 Arquivo: $BUILD_DIR/${APP_NAME}-${APP_VERSION}-standalone-x86_64.AppImage"
    echo "📏 Tamanho: $(ls -lh "$BUILD_DIR/${APP_NAME}-${APP_VERSION}-standalone-x86_64.AppImage" | awk '{print $5}')"
    echo ""
    echo "🚀 Para testar:"
    echo "  $BUILD_DIR/${APP_NAME}-${APP_VERSION}-standalone-x86_64.AppImage"
    echo ""
    echo "✨ Este AppImage inclui ADB e scrcpy - funciona sem dependências externas!"
    echo ""
else
    echo "❌ Erro ao criar AppImage Standalone"
    exit 1
fi
